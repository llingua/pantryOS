...
start_simple() {
    ...
    # Avvia server in background completamente separato
...
start_complete() {
    ...
    # Avvia server completo in background completamente separato
    node server/pantryos-server.js > /dev/null 2>&1 &
    SERVER_PID=$!
    disown $SERVER_PID
